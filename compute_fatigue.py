...
def main():
    ...
    if skip_if_not_in_window("compute_fatigue"):
        return
    # Always show 5 days PRECEDING the next upcoming game day.
    # - Before 4am ET: next game day = today (morning, today's games haven't
    #   started yet). Window = [today-5 .. today-1]  (excludes today)
    # - From 4am ET through midnight: next game day = tomorrow (today's games
    #   are in progress or done, and we want to see the state heading into
    #   tomorrow). Window = [today-4 .. today]  (includes today)
    # The window flips at midnight ET automatically. The target_date field
    # in the output tells the frontend which game day this usage applies to.
    et_now = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(hours=4)
    today = et_now.date()
    if et_now.hour < 4:
        target = today                                         # late-night / overnight
    else:
        target = today + datetime.timedelta(days=1)            # evening/overnight → tomorrow
    dates = [(target - datetime.timedelta(days=d)).isoformat() for d in range(5, 0, -1)]
    print(f"Building fatigue for target {target.isoformat()} · window: {dates[0]} .. {dates[-1]}")
